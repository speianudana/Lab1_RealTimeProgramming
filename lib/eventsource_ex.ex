...
#saves in local cache the messages
  defp dispatch(_, message) do
    ...
    if  message.data =~ "_sensor" do
      if (CacheServer.read(:messages)==nil) do
        CacheServer.write(:messages, [message])
      else
        CacheServer.write(:messages,Enum.concat(CacheServer.read(:messages), [message]))
        :timer.sleep(100)
      end
    end
  end
